...
- **Масштабирование**: 1:120
- **Раздельные оси** (бранч `separate-scroll-speed-v-h`): вертикальный 1:1, горизонтальный 1:3
...
- **Раздельные оси** (бранч `separate-scroll-speed-v-h`): вертикальный 1:1, горизонтальный 1:4
- **Инверсия**: Y-ось инвертирована
...
// Медленный скролл (с раздельными осями в бранче separate-scroll-speed-v-h)
scroll_slow {
    layers = <NUM_AND_FUN>;
    input-processors = <&zip_xy_scaler 1 120>,  // Очень медленная прокрутка
                       <&zip_wheel_scaler 1 1>,    // Вертикальный скролл
                       <&zip_hwheel_scaler 1 3>;   // Горизонтальный в 3 раза медленнее
};

// Быстрый скролл (с раздельными осями в бранче separate-scroll-speed-v-h)
scroll {
    layers = <QWERTY>;
    input-processors = <&zip_xy_scaler 1 40>,   // Быстрая прокрутка
                       <&zip_wheel_scaler 1 1>,    // Вертикальный скролл
                       <&zip_hwheel_scaler 1 4>;   // Горизонтальный в 4 раза медленнее
};
```
